...
/**
 * @brief estimate exposure time & visualize results
 * 
 * @return double 
 */
double RapidExposureTimeEstimator::estimateExposureTimeAndVisualize(std::string saved_path)
{
    /*
     * A minimum number of m_window_size frames is required for exposure time estimation
     * Not enough frames available yet, fix exposure time to 1.0
    ...
    // Traverse all features in the last frame
    for(int i = 0;i < features_last_frame.size();i++)
    {
        // Only features tracked from previous frame will be used to exposure time estimate
        //skip newly extracted features (no link to previous frame, cannot be used for exp. estimation)
        if(features_last_frame.at(i)->m_prev_feature == NULL)
            continue;
        ...
        // Todo: k should start from 1?
        //* Find the starting point of the current feature tracking in the window 
        //* and accumulate the radiance in the feature patch.
        for(int k = 0;k < m_window_size;k++)
        {
            // Go one feature backwards
            curr_feature = curr_feature->m_prev_feature;
    ...
    cv::Mat corrected_image = m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_image_corrected;
    cv::Mat original_image  = m_database->m_tracked_frames.at(m_database->m_tracked_frames.size()-1).m_image;
    //* Correct the corrected image by the final exposure estimateS
    corrected_image /= final_exp_estimate;
    ...
    // white-normal red-overexposed blue-underexposed
    cv::Mat exposure_status(corrected_image.size(), CV_8UC3, cv::Scalar(255, 255, 255));

    // TODO: Determine whether overexposed or underexposed by pixel difference
    for(int r = 0;r < corrected_image.rows;r++)
    {
        for(int c = 0;c < corrected_image.cols;c++)
        {
            ...
            {
                nr_underexposed++;
                exposure_status.at<cv::Vec3b>(r, c) = cv::Vec3b(255, 0, 0);
            }
            else if(image_value > 240 && orig_value <= 200)
            {
                nr_overexposed++;
                exposure_status.at<cv::Vec3b>(r, c) = cv::Vec3b(0, 0, 255);
            }
        }
    }
    
    std::string exposure_status_path = saved_path + "exposure_status.png";
    // cv::imshow("Exposure Status", exposure_status);
    // cv::waitKey(0);
    cv::imwrite(exposure_status_path, exposure_status);
    std::cout << "Save exposure status to " << exposure_status_path << std::endl;
